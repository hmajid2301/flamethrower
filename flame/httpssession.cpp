...
void HTTPSSession::add_stream(http2_stream_data *stream_data)
{
    stream_data->next = _current_session->root.next;
...
http2_stream_data* HTTPSSession::create_http2_stream_data(std::unique_ptr<char[]> data, size_t len)
{
    ...
    http2_stream_data *prev = {};
    http2_stream_data *next = {};
	std::string streamData(data.get(), len);
    http2_stream_data *root = new http2_stream_data(prev, next, uri, u, authority, path, stream_id, streamData);
    add_stream(root);
    return root;
}

void HTTPSSession::initiate_session_data()
{
    std::string uri = _target.uri;
    struct http_parser_url *u = _target.parsed;
    std::string authority(&uri[u->field_data[UF_HOST].off], u->field_data[UF_HOST].len);
    ...
    http2_stream_data *next = {};
	std::string data = "";
    http2_stream_data stream_data = {prev, next, uri, u, authority, path, stream_id, data};
    _current_session = new http2_session_data(session, stream_data);
}

http2_session_data* HTTPSSession::getSession()
{
...
void HTTPSSession::init_nghttp2()
{
    ...
    nghttp2_session_client_new(&_current_session->session, callbacks, this);
...
{
    ssize_t stream_id = nghttp2_session_mem_recv(_current_session->session, (const uint8_t *) data, len);
    if (stream_id < 0) {
...
{
    int rv;
    rv = nghttp2_session_send(_current_session->session);
    if (rv != 0) {
